...
export interface AnimateAsset {
	...
	/**
	 * Background color of the scene, as defined in when the asset is published.
	 */
    background: number,
	/**
	 * Width of the scene, as defined in when the asset is published.
	 */
    width: number,
	/**
	 * Height of the scene, as defined in when the asset is published.
	 */
    height: number,
	/**
	 * Framerate of the scene (frames per second), as defined in when the asset is published.
	 */
    framerate: number,
	/**
	...
	/**
	 * Function for getting a texture by ID. This is initially null, and must be set before any
	 * items are instantiated.
	 * It can be set to PIXI.Texture.from, or animate.load() will set it up to pull from just the
